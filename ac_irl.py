...
class AC_IRL:
    ...
    def __init__(self, theta=8.64, shift=0, alpha_scale=1e4, d=30, lr_reward=1e-4, num_policies=10, c=2e11, reg='dropout_l1l2', n_fc3=8, n_fc4=4, saved_network=None, use_tf=True, summarize=False):
        """
    ...
    def read_demonstrations(self, state_dir, action_dir, dim_action=20, start_day=1):
        ...
        for idx_day in range(start_day, start_day+num_file_action):
            ...
            next_state = states[0,0:self.d]
            last_action = np.zeros([self.d,self.d])
            file_new = 'data_new_P/trends%d.txt' % idx_day
            for hour in range(0,6):
                state = states[hour, 0:self.d]
                if(hour == 5):
                    action = last_action
                else:  
                    next_state = states[hour+1, 0:self.d]
                    action = np.matmul(np.linalg.pinv([state]),[next_state])   
                    for r in range(0,self.d):
                        if np.count_nonzero(action[r, :]) == 0:
                            action[r,r] = 1.0
                    #action = action / np.sum(action, axis=1, keepdims=True)                    
                trajectory.append( (state, action) )
                # with open(file_new, 'ab') as f :
                #     np.savetxt(f, action,fmt = '%.3e')
                #     f.write(("\n").encode())
                last_action = action
...
if __name__ == "__main__":
    ...
    n_fc3 = 4
    n_fc4 = 8
    ac = AC_IRL(theta=17, reg='l1l2', n_fc3=4, n_fc4=8, saved_network='model_%s_%d_%d.ckpt' % (reg, n_fc3, n_fc4))

    ac.visualize_test(theta = 17)
